...
async function loadDeltapagSubscriptions() {
    try {
        ...
        if (!tbody) {
            console.error('❌ Elemento deltapag-subscriptions-tbody não encontrado!');
            return;
        }
        
        console.log(`✅ Renderizando ${subs.length} assinaturas na tabela...`);
...
function closeQRCodeModal() {
    ...
    // NÃO limpar currentQRData aqui - manter para download funcionar
    // currentQRData = null;
}
...
// Função para abrir modal de assinatura (que estava faltando)
function openDeltapagModal() {
    const modal = document.getElementById('deltapag-modal');
    if (modal) {
        modal.classList.remove('hidden');
    }
}
...
function closeDeltapagModal() {
    ...
    if (modal) {
        ...
        document.getElementById('deltapag-result').innerHTML = '';
        document.getElementById('deltapag-result').classList.add('hidden');
    }
}

// Função para submeter formulário de assinatura DeltaPag
async function submitDeltapagForm(e) {
    e.preventDefault();
    
    console.log('🔵 submitDeltapagForm CHAMADO');
    ...
    const formData = {
        customer: {
            name: document.getElementById('deltapag-customer-name').value,
            email: document.getElementById('deltapag-customer-email').value,
            cpf: document.getElementById('deltapag-customer-cpf').value.replace(/\D/g, ''),
            phone: document.getElementById('deltapag-customer-phone').value
        },
        card: {
            number: document.getElementById('deltapag-card-number').value.replace(/\s/g, ''),
            holderName: document.getElementById('deltapag-card-holder').value,
            expiryMonth: document.getElementById('deltapag-card-month').value,
            expiryYear: document.getElementById('deltapag-card-year').value,
            cvv: document.getElementById('deltapag-card-cvv').value
        },
        subscription: {
            value: parseFloat(document.getElementById('deltapag-value').value),
            cycle: document.getElementById('deltapag-recurrence').value,
            description: document.getElementById('deltapag-description').value
        },
        split: {
            walletId: document.getElementById('deltapag-split-wallet').value,
            percentage: parseFloat(document.getElementById('deltapag-split-percentage').value) || 0
        }
    };
    ...
    try {
        // Chamar API
        const response = await axios.post('/api/admin/deltapag/subscription', formData);
            ...
            resultDiv.innerHTML = `
                <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1">
                            <h4 class="font-bold text-green-900 mb-2">✅ Assinatura Criada com Sucesso!</h4>
                            <p class="text-sm text-green-800 mb-2">
                                Cliente: <strong>${formData.customer.name}</strong><br>
                                Valor: <strong>R$ ${formData.subscription.value.toFixed(2)}</strong><br>
                                Recorrência: <strong>${formData.subscription.cycle}</strong>
                            </p>
                            ${response.data.data?.chargeId ? `<p class="text-xs text-green-700">ID da Cobrança: ${response.data.data.chargeId}</p>` : ''}
                        </div>
                    </div>
                </div>
            `;
            ...
            <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <button onclick="submitDeltapagForm(event)" 
...
// Exportar funções de modais deltapag
window.openDeltapagModal = openDeltapagModal;
window.closeDeltapagModal = closeDeltapagModal;
window.submitDeltapagForm = submitDeltapagForm;
window.openDeltapagLinkModal = openDeltapagLinkModal;
window.closeDeltapagLinkModal = closeDeltapagLinkModal;
window.showDeltapagLinksModal = showDeltapagLinksModal;
window.closeDeltapagLinksModal = closeDeltapagLinksModal;
window.openDeltapagImportModal = openDeltapagImportModal;
window.closeDeltapagImportModal = closeDeltapagImportModal;

// Adicionar event listener ao formulário quando o DOM carregar
document.addEventListener('DOMContentLoaded', function() {
    const form = document.getElementById('deltapag-form');
    if (form) {
        form.addEventListener('submit', submitDeltapagForm);
        console.log('✅ Event listener do formulário DeltaPag adicionado');
    }
    ...
    const phoneInput = document.getElementById('deltapag-customer-phone');
    if (phoneInput) {
    ...
    const cardInput = document.getElementById('deltapag-card-number');
    if (cardInput) {
    ...
    const holderInput = document.getElementById('deltapag-card-holder');
    if (holderInput) {
...
console.log('✅ Funções de modais deltapag:', {
    openDeltapagModal: typeof window.openDeltapagModal,
    openDeltapagLinkModal: typeof window.openDeltapagLinkModal,
    showDeltapagLinksModal: typeof window.showDeltapagLinksModal,
    openDeltapagImportModal: typeof window.openDeltapagImportModal
});
